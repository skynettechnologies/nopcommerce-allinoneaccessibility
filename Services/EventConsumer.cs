...
namespace Nop.Plugin.Widgets.AllinOneAccessibility.Services
{
    /// <summary>
        ...
        IConsumer<OrderPlacedEvent>
    {
        ...
        public async Task HandleEventAsync(EntityDeletedEvent<Address> eventMessage)
        {
            ...
            await _genericAttributeService.SaveAttributeAsync<string>(address, AllinOneAccessibilityDefaults.AddressValueAttribute, null);
        }
        ...
        public async Task HandleEventAsync(EntityInsertedEvent<CustomerAddressMapping> eventMessage)
        {
            ...
            if (_httpContextAccessor.HttpContext.Items.TryGetValue(AllinOneAccessibilityDefaults.AddressValueAttribute, out var addressValue))
            {
                var address = await _addressService.GetAddressByIdAsync(mapping.AddressId);
                if (address is not null)
                    await _genericAttributeService.SaveAttributeAsync(address, AllinOneAccessibilityDefaults.AddressValueAttribute, addressValue);
            }
        }
        ...
        public async Task HandleEventAsync(ModelReceivedEvent<BaseNopModel> eventMessage)
        {
            ...
            if (!await _widgetPluginManager.IsPluginActiveAsync(AllinOneAccessibilityDefaults.SystemName, customer))
                return;

            if (!_allinOneAccessibilitySettings.Enabled)
                return;

            //cache the value within the request, we save it to the address later
            var form = _httpContextAccessor.HttpContext.Request.Form;
            if (form.TryGetValue(AllinOneAccessibilityDefaults.ComponentName, out var addressValue) && !StringValues.IsNullOrEmpty(addressValue))
                _httpContextAccessor.HttpContext.Items[AllinOneAccessibilityDefaults.AddressValueAttribute] = addressValue.ToString().TrimStart('/');
        }
        ...
        public async Task HandleEventAsync(OrderPlacedEvent eventMessage)
        {
            ...
            async Task copyAddressValueAsync(int? customerAddressId, int? orderAddressId)
            {
                ...
                var addressValue = customerAddress is not null
                    ? await _genericAttributeService.GetAttributeAsync<string>(customerAddress, AllinOneAccessibilityDefaults.AddressValueAttribute)
                    : null;
                if (!string.IsNullOrEmpty(addressValue))
                {
                    var orderAddress = await _addressService.GetAddressByIdAsync(orderAddressId ?? 0);
                    if (orderAddress is not null)
                        await _genericAttributeService.SaveAttributeAsync(orderAddress, AllinOneAccessibilityDefaults.AddressValueAttribute, addressValue);
                }
            }
